...
class PhysicsController {
    ...
    checkCollision(direction, speed) {
        ...
        // Check if we hit a step we can climb onto
        if (!this.isJumping && !this.isFalling) {
            ...
            if (hitObject?.userData?.isWall && hitObject?.userData?.blockHeight > 0) {
                ...
                if (this.debug) {
                    console.log("Hit raised block:", {
                        ...
                        diff: heightDiff,
                        maxStepHeight: this.stepHeight
                    });
                }

                // Allow stepping up if height difference is within step height limit
                // Rather than requiring EXACTLY half-block, accept anything within the limit
                if (heightDiff > 0 && heightDiff <= this.stepHeight) {
    ...
    checkForwardCollision(direction, speed) {
        ...
        // Cast rays at multiple heights to check for collisions
        // For stepping, we'll check at foot and chest level
        const footRayHeight = 0.1; // Near feet
        const chestRayHeight = 1.0; // Around chest level

        // Cast ray at foot level
        const footRayStart = new THREE.Vector3(
            playerPos.x,
            this.currentGroundHeight + footRayHeight,
            playerPos.z
        );

        const footRaycaster = new THREE.Raycaster(
            footRayStart,
            horizontalDirection,
            0,
            effectiveDistance
        ...
        // Cast ray at chest level
        const chestRayStart = new THREE.Vector3(
            playerPos.x,
            this.currentGroundHeight + chestRayHeight,
            playerPos.z
        );
        ...
        // Get objects that might be steps
        const steppableObjects = this.scene3D.scene.children.filter(obj =>
            obj.userData?.isWall ||
            obj.userData?.isRaisedBlock
        );

        // Check foot-level collisions
        const footHits = footRaycaster.intersectObjects(steppableObjects);

        if (footHits.length > 0) {
            const hitObject = footHits[0].object;

            // If it's a potentially steppable object
            if (hitObject.userData.isRaisedBlock ||
                (hitObject.userData.isWall && hitObject.userData.blockHeight > 0)) {

                const heightDiff = hitObject.userData.blockHeight - this.currentGroundHeight;

                // Check if the height difference is within step range
                if (heightDiff > 0 && heightDiff <= this.stepHeight) {
                    // Check chest-level collisions to make sure we can fit
                    const chestHits = chestRaycaster.intersectObjects(steppableObjects);

                    // If nothing at chest level or it's further away than the foot hit,
                    // then we can step up onto this object
                    if (chestHits.length === 0 || chestHits[0].distance > footHits[0].distance) {
                        return {
                            canMove: true,
                            hitObject: hitObject,
                            distance: footHits[0].distance
                        };
                    }
                }
            }

            // Regular collision - can't move through this object
            return {
                canMove: false,
                hitObject: hitObject,
                distance: footHits[0].distance
    ...
    updatePlayerHeight(direction, speed) {
        ...
        if (heightDiff < 0) {
            ...
            this.isFalling = true;
            return true;
        } else if (heightDiff > 0 && heightDiff <= this.stepHeight) {
            // Allow stepping up if within step height (not just exactly one step)
            this.currentGroundHeight = groundHeight;
            return true;
    ...
    checkForAutoStep(direction, speed) {
        const playerPos = this.scene3D.camera.position;

        // First, check if we're already in a jumping or falling state
        if (this.isJumping || this.isFalling) {
            return false;
        }

        // Use horizontal direction only for the ray
        const horizontalDirection = new THREE.Vector3(direction.x, 0, direction.z).normalize();

        // Calculate the check distance - slightly beyond our normal movement
        const checkDistance = speed + 0.3; // Add a bit more than our normal collision buffer

        // Ray starting position - at foot level
        const footRayStart = new THREE.Vector3(
            playerPos.x,
            this.currentGroundHeight + 0.1, // Slightly above current ground
            playerPos.z
        );

        // Cast a ray at foot level to detect the step
        const footRaycaster = new THREE.Raycaster(
            footRayStart,
            horizontalDirection,
            0,
        ...
        // Find objects we can potentially step on
        const steppableObjects = this.scene3D.scene.children.filter(obj =>
            obj.userData?.isWall ||
            obj.userData?.isRaisedBlock ||
            obj.userData?.blockHeight !== undefined
        ...
        // Check if there's anything blocking our path at foot level
        const footHits = footRaycaster.intersectObjects(steppableObjects);

        if (footHits.length > 0) {
            const hitObject = footHits[0].object;

            // Check if this is a raised block or has a block height
            if (hitObject.userData?.isRaisedBlock || hitObject.userData?.blockHeight !== undefined) {
                const stepHeight = hitObject.userData.blockHeight || 0;

                // Calculate height difference between current position and step
                const heightDiff = stepHeight - this.currentGroundHeight;

                // If the height difference is within our step height limit
                if (heightDiff > 0 && heightDiff <= this.stepHeight) {
                    // Cast another ray at chest level to make sure we can fit
                    const chestRayStart = new THREE.Vector3(
                        playerPos.x,
                        this.currentGroundHeight + 1.0, // Around chest height
                        playerPos.z
                    );
                    ...
                    // If there's nothing blocking at chest level or it's farther than foot hit
                    if (chestHits.length === 0 || chestHits[0].distance > footHits[0].distance) {
                        if (this.debug) {
                            console.log(`Auto-stepping from height ${this.currentGroundHeight} to ${stepHeight}`);
                        }

                        // We can step up! Update the ground height
                        this.currentGroundHeight = stepHeight;
                        return true;
                    }
                }
            }
